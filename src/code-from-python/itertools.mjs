'use strict';
import {list, tuple, len, __kwargtrans__, set} from "./org.transcrypt.__runtime__.mjs";
...
export var cycle = function* (iterable) {
    ...
    while (true) for (let item of buffer) yield item
};
export var repeat = function* (item, n) {
    if (typeof n == "undefined") while (true) yield item; else for (let index = 0; index < n; index++) yield item
};
export var accumulate =
    function* (iterable, func) {
        let sum;
        let first = true;
        if (func) for (let item of iterable) {
            if (first) {
                sum = item;
                first = false
            } else sum = func(sum, item);
            yield sum
        } else for (let item of iterable) {
            if (first) {
                sum = item;
                first = false
            } else sum = sum + item;
...
export var chain = function* () {
    ...
    for (let arg of args) for (let item of arg) yield item
};
chain.from_iterable = function* (iterable) {
    for (let item of iterable) for (let subItem of item) yield subItem
};
export var compress = function* (data, selectors) {
    let dataIterator =
        data[Symbol.iterator].call(data);
    let selectorsIterator = selectors[Symbol.iterator]();
    while (true) {
        let dataItem = dataIterator.next();
        let selectorsItem = selectorsIterator.next();
        if (dataItem.done || selectorsItem.done) break; else if (selectorsItem.value) yield dataItem.value
    }
};
export var dropwhile = function* (pred, seq) {
    let started = false;
    for (let item of seq) if (started) yield item; else if (!pred(item)) {
        started = true;
        yield item
    }
};
export var filterfalse = function* (pred, seq) {
    for (let item of seq) if (!pred(item)) yield item
};
export var groupby = function* (iterable, keyfunc) {
    ...
    while (more) yield tuple([groupKey, group()])
};
export var islice = function* () {
    ...
    if (args.length == 2) {
        ...
        start = args[1];
        stop = args[2];
        if (args.length == 4) step = args[3]; else step = 1
    }
    for (let index = 0; index < start; index++) if (anIterator.next().done) return;
    for (let index = 0; index < stop - start; index++) {
        let next = anIterator.next();
        if (next.done) return;
        if (index % step == 0) yield next.value
    }
};
export var starmap = function* (func, seq) {
    ...
    while (true) {
        let next = anIterator.next();
        if (next.done) return; else yield func(...next.value)
    }
};
export var takewhile = function* (pred, seq) {
    for (let item of seq) if (pred(item)) yield item; else return
};
export var tee = function (iterable, n) {
    if (n == undefined) n = 2;
    let all = [];
    let one = list(iterable);
    for (let i = 0; i < n; i++) all.append(one[Symbol.iterator]());
    return list(all)
};
...
export var combinations_with_replacement = function (iterable, r) {
    ...
    function recurse(tail, molecule, rNext) {
        for (let index = 0; index < len(tail); index++) {
            let newMolecule =
                molecule.concat(tail.slice(index, index + 1));
            if (rNext) recurse(tail.slice(index), newMolecule, rNext - 1); else result.append(tuple(newMolecule))
        }
    }
